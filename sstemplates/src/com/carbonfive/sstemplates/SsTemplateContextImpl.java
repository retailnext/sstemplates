    ...
    implements SsTemplateContext
{
  ...
  public void incrementCellIndex()
  {
    ...
    for ( Region region = getRegionForCurrentLocation(); region != null; region = getRegionForCurrentLocation() )
    {
      if (( columnIndex != region.getColumnFrom() ) || ( rowIndex != region.getRowFrom() ))
        setColumnIndex(region.getColumnTo() + 1);
    }
  }
  ...
  public Region getRegionForCurrentLocation()
  {
    for (int i=0; i < sheet.getNumMergedRegions(); i++ )
    {
      if ( sheet.getMergedRegionAt(i).contains( rowIndex, (short) columnIndex ) )
